...
export const cleanlinessEnvironmentSchema = z.object({
  streetCleanlinessRating: z
    .number()
    .min(1, "Please select a rating")
    .max(5, "Rating must be between 1 and 5"),
  trashManagementRating: z
    .number()
    .min(1, "Please select a rating")
    .max(5, "Rating must be between 1 and 5"),
  parksQualityRating: z
    .number()
    .min(1, "Please select a rating")
    .max(5, "Rating must be between 1 and 5"),
...
export const foodAmenitiesSchema = z.object({
  ...
  restaurantVarietyRating: z
    .number()
    .min(1, "Please select a rating")
    .max(5, "Rating must be between 1 and 5"),
  foodAffordabilityRating: z
    .number()
    .min(1, "Please select a rating")
    .max(5, "Rating must be between 1 and 5"),
...
export const financialsSchema = z.object({
  rentAffordability: z.enum(
    ["Very affordable", "Affordable", "Fair", "Expensive", "Very expensive"],
    {
...
export type CleanlinessEnvironmentData = z.infer<
  typeof cleanlinessEnvironmentSchema
>;
